...
db.exec(`
  ...
  -- COURSES (UPDATED SCHEMA: Structured Schedule)
  CREATE TABLE IF NOT EXISTS courses (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    code TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
    ...
    -- NEW STRUCTURED SCHEDULE COLUMNS
    theory_days TEXT NOT NULL,   -- e.g., "MW"
    theory_time TEXT NOT NULL,   -- e.g., "08:30 - 10:00"
    lab_day TEXT,                -- e.g., "Tue" (Nullable)
    lab_time TEXT,               -- e.g., "10:00 - 12:00" (Nullable)
  ...
  -- *** NEW: REFERENCE SCHEDULE RULES ***
  CREATE TABLE IF NOT EXISTS schedule_rules (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    category TEXT NOT NULL, -- 'theory_day', 'lab_day', 'theory_slot', 'lab_slot_2h', 'lab_slot_3h'
    value TEXT NOT NULL,    -- The value stored in DB (e.g., 'MW', '08:30 - 10:00')
    display TEXT NOT NULL   -- The user-friendly text
  );
`);
...
if (studentCount.count === 0) {
  ...
  db.prepare(
    `INSERT INTO admins (email, password, name) VALUES ('nura@gmail.com', '123456', 'M. Nura Alam Naim')`
  ).run();
  ...
  // --- SEED FIXED SCHEDULES (The "Rules") ---
  const ruleStmt = db.prepare("INSERT INTO schedule_rules (category, value, display) VALUES (?, ?, ?)");

  // Theory Days (Strict: ST, SR, MW, TR)
  ruleStmt.run("theory_day", "MW", "MW (Mon,Wed)");
  ruleStmt.run("theory_day", "ST", "ST (Sun,Tue)");
  ruleStmt.run("theory_day", "SR", "SR (Sun,,Thu)");
  ruleStmt.run("theory_day", "TR", "TR (Tue,Thu)");

  // Lab Days (Sun-Thu)
  ["S", "M", "T", "W", "T"].forEach(d => {
    ruleStmt.run("lab_day", d.substring(0, 3), d);
  });

  // Theory Slots (Start 8:30, 90mins, 10min break)
  // 08:30-10:00, 10:10-11:40, 11:50-01:20, 01:30-03:00, 03:10-04:40, 04:50-06:20
  const theorySlots = [
    "08:30 - 10:00", "10:10 - 11:40", "11:50 - 01:20",
    "01:30 - 03:00", "03:10 - 04:40", "04:50 - 06:20"
  ];
  theorySlots.forEach((t, i) => ruleStmt.run("theory_slot", t, `Slot ${i + 1}: ${t}`));

  // Lab Slots (Start 8:00)
  // 4 Credits = 2 Hours
  const lab2h = ["08:00 - 10:00", "10:10 - 12:10", "12:20 - 02:20", "02:30 - 04:30", "04:40 - 06:40"];
  lab2h.forEach(t => ruleStmt.run("lab_slot_2h", t, t));

  // 4.5 Credits = 3 Hours
  const lab3h = ["08:00 - 11:00", "11:10 - 02:10", "02:20 - 05:20"];
  lab3h.forEach(t => ruleStmt.run("lab_slot_3h", t, t));

  console.log("Fixed Schedule Rules Seeded.");

  // C. Insert Courses (Updated for Structured Schedule)
  const insertCourse = db.prepare(`
  ...
  const courses = [
    // CSE (Code 60)
    ["CSE101", "Intro to Computer Science", "CSE", 3, "Dr. Ada Lovelace", "MW", "08:30 - 10:00", null, null],
    ["CSE201", "Object Oriented Programming", "CSE", 3, "Dr. Grace Hopper", "ST", "10:10 - 11:40", null, null],
    ["CSE301", "Database Systems", "CSE", 3, "Dr. Robert Brown", "SR", "01:30 - 03:00", null, null],
    // EEE (Code 50)
    ["EEE101", "Electrical Circuits I", "EEE", 3, "Dr. Tesla", "TR", "08:30 - 10:00", null, null],
    ["EEE201", "Electronics I", "EEE", 3, "Dr. Edison", "MW", "11:50 - 01:20", null, null],
    // BBA (Code 40)
    ["ACT201", "Financial Accounting", "BBA", 3, "Prof. Ledger", "ST", "03:10 - 04:40", null, null],
    ["MGT301", "Principles of Management", "BBA", 3, "Dr. Drucker", "SR", "04:50 - 06:20", null, null],
    // ENG (Code 20)
    ["ENG101", "English Composition", "ENG", 3, "Prof. Shakespeare", "TR", "10:10 - 11:40", null, null],
  ];

  const allCourseIds = [];
  courses.forEach((c) => {
    const res = insertCourse.run(
      c[0], c[1], c[2], c[3], c[4], "faculty@san.edu", // Default fallback email
      c[5], c[6], c[7], c[8], // Schedule Parts
      "AB1-101", 1, "Fall-2025"
    );
    allCourseIds.push({ id: res.lastInsertRowid, credits: c[3] });
  });
